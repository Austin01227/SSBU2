...
if (run)
{
	switch(_phase)
	{
		...
		{
			//Animation
			anim_sprite = spr_kirby_dsmash;	//REPLACE
			anim_frame = 0;
			anim_speed = 0;
		...
		case 1:
		{
			if (attack_frame == 0)
			{
				...
				create_melee(0, 30, 1.5, 0.5, _damage, 8, 0.5, 10, 29, 5, HITBOX_SHAPE.circle, 0, FLIPPER.from_player_center_horizontal);
			}
			break;
		}
		//Sourspot
		case 2:
		{
			if (attack_frame == 0)
			{
				...
				create_melee(0, 30, 1.4, 0.5, _damage, 7, 0.5, 9, 29, 8, HITBOX_SHAPE.circle, 0, FLIPPER.from_player_center_horizontal);
			}
			break;
		}
		//Endlag
